...
module Openapi
  module Entities
    class CoreChurnModelDataItem < Grape::Entity
    ...
    # Core Churn 「获取模型数据」列表：结构与 fetch_metric_data_v2 一致，items 含本模块全部指标字段及 score
    class CoreChurnModelDataResponse < Grape::Entity
      ...
      expose :items, using: Entities::CoreChurnModelDataItem,
             documentation: { type: 'CoreChurnModelDataItem', desc: 'response', param_type: 'body', is_array: true }
    end
  end
end